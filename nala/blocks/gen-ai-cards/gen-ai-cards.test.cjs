...
import { features } from './gen-ai-cards.spec.cjs';
import GenAICards from './gen-ai-cards.page.cjs';
...
test.describe('gen-ai-cards test suite', () => {
  ...
  features[0].path.forEach((path) => {
    test(`${features[0].name}, path: ${path}, test gen-ai cards homepage`, async ({ baseURL, page }) => {
  ...
  features[1].path.forEach((path) => {
    test(`${features[1].name}, path: ${path}, test gen-ai-card with prompt text`, async ({ baseURL, page }) => {
      ...
      await test.step('validate elements in block ', async () => {
        ...
        await expect(genAICards.prevButton).toHaveClass(/arrow-hidden/);
        await expect(genAICards.nextButton).not.toHaveClass(/arrow-hidden/);
      });
