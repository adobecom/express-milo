...
const { features } = require('./frictionless-quick-action.spec.cjs');
const FrictionlessQuickActionBlock = require('./frictionless-quick-action.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

test.describe('FrictionlessQuickActionBlock Test Suite', () => {
  // Test Id : 0 : @frictionless-quick-action-default
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}`;
    const block = new FrictionlessQuickActionBlock(page, features[0].selector);
