...
export const loadFaasFiles = async () => {
...
const afterSubmitCallback = (e) => {
  ...
  if (dialogModal) {
    ...
    if (faasFormWrapper) {
      ...
      const checkIcon = getIconElementDeprecated('checkmark-green');
...
export const initFaas = async (config, targetEl) => {
  ...
  if (window.location.pathname === '/tools/axfaas') {
    state.as = false;
    state.ar = false;
  }
