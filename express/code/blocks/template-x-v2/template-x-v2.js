...
function populateTemplates(block, props, templates) {
  for (let tmplt of templates) {
    ...
    const innerWrapper = block.querySelector('.template-x-v2-inner-wrapper');
...
async function decorateNewTemplates(block, props, options = { reDrawMasonry: false }) {
  ...
  if (block.classList.contains(TWO_ROW)) {
    await build2by2(block.querySelector('.template-x-v2-inner-wrapper'), block);
  }
...
function updateLottieStatus() {}

async function appendCategoryTemplatesCount() {}

async function decorateCategoryList() {}
...
function toggleMasonryView(block, props, button, toggleButtons) {
  const blockEl = block.closest('.template-x-v2');
  blockEl.classList.add('template-x-v2-wrapper');
  const templatesToView = block.querySelectorAll('.template:not(.placeholder)');
  const blockWrapper = blockEl.closest('.template-x-v2-wrapper');
...
function initExpandCollapseToolbar(block, templateTitle, toggle, toggleChev) {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (e.target.closest('.template-x-v2.holiday') || (
...
async function decorateTemplates(block, props) {
  ...
  if (props.orientation.toLowerCase() !== 'horizontal') {
    if (rows > 6 || block.classList.contains('sixcols') || block.classList.contains('fullwidth')) {
      ...
      block.classList.add('template-x-v2-complete');
    }
  }
...
function renderFallbackMsgWrapper(block, { fallbackMsg }) {
  let fallbackMsgWrapper = block.querySelector('.template-x-v2-fallback-msg-wrapper');
  if (!fallbackMsgWrapper) {
    fallbackMsgWrapper = createTag('div', { class: 'template-x-v2-fallback-msg-wrapper' });
...
async function buildTemplateList(block, props, type = []) {
  ...
  if (templates?.length > 0) {
    ...
    renderFallbackMsgWrapper(block, props);
    const blockInnerWrapper = createTag('div', { class: 'template-x-v2-inner-wrapper' });
  ...
  if (templates && props.tabs) {
    ...
    const templatesWrapper = block.querySelector('.template-x-v2-inner-wrapper');
...
export default async function decorate(block) {
  ...
  block.dataset.blockName = 'template-x-v2';
