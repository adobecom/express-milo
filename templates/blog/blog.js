...
// const { createTag, getMetadatadata } = await import(`${getLibs()}/utils/utils.js`);
/*
...
async function decorateBlogPage() {
  ...
  if ($h1 && author && date) {
    ...
    const $heroSection = createTag('div', { class: 'section hero' });
    const $div = createTag('div');
    ...
    const language = getConfig().locale?.ietf || 'en-US'; //getLanguage(getLocale(config.locales));
